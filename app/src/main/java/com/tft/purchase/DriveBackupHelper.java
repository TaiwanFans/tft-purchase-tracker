...
public class DriveBackupHelper {
    private static final long AUTO_INTERVAL_MS = 20L * 60L * 60L * 1000L;

    public static void backupIfDueAsync(Context context) {
        Context app = context.getApplicationContext();
        new Thread(() -> {
            try {
                SharedPreferences prefs = app.getSharedPreferences("tft_settings", Context.MODE_PRIVATE);
                if (prefs.getString("backup_tree_uri", "").isEmpty()) return;
                long last = prefs.getLong("last_backup_ms", 0L);
    ...
    public static String backupNow(Context context) throws Exception {
        ...
        String treeString = prefs.getString("backup_tree_uri", "");
        if (treeString == null || treeString.isEmpty()) throw new Exception("尚未選擇 Google Drive 備份資料夾");
        Uri treeUri = Uri.parse(treeString);
        ...
        String fileName = "全益採購追蹤備份_" + stamp + ".zip";
        Uri outUri = DocumentsContract.createDocument(context.getContentResolver(), parent, "application/zip", fileName);
        if (outUri == null) throw new Exception("無法在所選資料夾建立備份檔");
        ...
        OutputStream raw = context.getContentResolver().openOutputStream(outUri, "w");
        if (raw == null) throw new Exception("無法寫入備份檔");
    ...
    public static int restore(Context context, Uri zipUri) throws Exception {
        ...
        try (InputStream closeRaw = raw; ZipInputStream zip = new ZipInputStream(closeRaw)) {
            ...
            while ((e = zip.getNextEntry()) != null) {
                ...
                if ("data.json".equals(name)) {
                    ...
                    int n; while ((n = zip.read(buf)) > 0) out.write(buf, 0, n);
        ...
        if (orders == null) throw new Exception("備份資料格式不正確");
        PurchaseDbHelper db = new PurchaseDbHelper(context);
        db.clearAll();
        int count = 0;
        for (int x = 0; x < orders.length(); x++) {
            ...
            p.vendorName = o.optString("vendor_name", "");
            p.location = o.optString("location", "");
            p.orderNo = o.optString("order_no", "");
            p.purchaseDate = o.optString("purchase_date", "");
            p.signatureReturnDate = o.optString("signature_return_date", "");
            p.notes = o.optString("notes", "");
            p.rawOcr = o.optString("raw_ocr", "");
            p.completed = o.optBoolean("completed", false);
            p.createdAt = o.optLong("created_at", System.currentTimeMillis());
            p.legacyItems = o.optString("legacy_items", "");
            p.legacyDeliveryDate = o.optString("legacy_delivery_date", "");
            p.imagePath = extracted.getOrDefault(o.optString("image_entry", ""), "");
            ...
            if (items != null) {
                for (int y = 0; y < items.length(); y++) {
                    ...
                    i.purchaseId = newId;
                    i.lineNo = j.optString("line_no", "");
                    i.description = j.optString("description", "");
                    i.quantity = j.optString("quantity", "");
                    i.unit = j.optString("unit", "");
                    i.unitPrice = j.optString("unit_price", "");
                    i.subtotal = j.optString("subtotal", "");
                    i.deliveryDate = j.optString("delivery_date", "");
                    i.note = j.optString("note", "");
                    i.completed = j.optBoolean("completed", false);
                    db.insertItem(i);
                }
            }
            count++;
        }
        if (root.has("reminder_days")) {
