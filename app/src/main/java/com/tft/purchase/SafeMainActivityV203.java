...
public class SafeMainActivityV203 extends TftActivityV203 {
    @Override
    ...
    private void showStartupFailure(Throwable error) {
        ...
        try {
            ...
            TextView info = label(details, 12, Color.rgb(15, 23, 42), false); info.setTextIsSelectable(true); info.setPadding(dp2(12), dp2(12), dp2(12), dp2(12)); info.setBackgroundColor(Color.WHITE); LinearLayout.LayoutParams ip = new LinearLayout.LayoutParams(-1, -2); ip.setMargins(0, dp2(16), 0, dp2(12)); page.addView(info, ip);
            ...
            Button restart = button("重新啟動 APP"); restart.setOnClickListener(v -> { Intent i = new Intent(this, SafeMainActivityV203.class); i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK); startActivity(i); finish(); }); page.addView(restart);
            ScrollView scroll = new ScrollView(this); scroll.addView(page); setContentView(scroll);
        } catch (Throwable ignored) {
            TextView v = new TextView(this); v.setText(details); v.setPadding(24, 48, 24, 24); setContentView(v);
        }
    }

    private String buildDetails(Throwable t) {
        StringBuilder s = new StringBuilder(); s.append("版本：2.0.3\n"); s.append("Android：").append(Build.VERSION.RELEASE).append(" / API ").append(Build.VERSION.SDK_INT).append("\n"); s.append("裝置：").append(Build.MANUFACTURER).append(' ').append(Build.MODEL).append("\n\n"); Throwable cur = t; int depth = 0;
        while (cur != null && depth < 5) { s.append(depth == 0 ? "錯誤：" : "Caused by：").append(cur.getClass().getName()).append("\n").append(String.valueOf(cur.getMessage())).append("\n"); StackTraceElement[] stack = cur.getStackTrace(); int max = Math.min(stack == null ? 0 : stack.length, 15); for (int i = 0; i < max; i++) s.append("  at ").append(stack[i]).append("\n"); cur = cur.getCause(); depth++; }
        return s.toString();
    }
