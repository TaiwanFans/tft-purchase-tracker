...
public class GemmaSetupActivity extends Activity {
    ...
    private void render() {
        ...
        tt.addView(label("分區放大 OCR + 局部 MiniCPM｜離線辨識",14,Color.rgb(37,99,235),true));
        ...
        card.addView(label("V2.0.16｜高解析分區＋衝突局部核對",18,Color.rgb(15,42,92),true));
        card.addView(label("採購單先做方向／紙張／透視校正、去陰影與去噪，再切 H1/H2/R1/R2/R3；每區放大後由 PP-OCRv6 Medium + Google ML Kit 各自掃描，再把座標拼回完整採購單。",15,Color.rgb(71,85,105),false));
        card.addView(label("MiniCPM 不再固定讀整張縮小 A4。只有 OCR 衝突或低信心座標會建立高解析局部核對圖；看不清楚時欄位留空並要求人工確認。",14,Color.rgb(22,101,52),true));
        card.addView(label("本機辨識知識庫會固定排除全益／台灣電扇等我方名稱。人工修正後才學習廠商、品名與規格；歷史資料只做校正提示，不會自動生成本次品項。",14,Color.rgb(22,101,52),true));
