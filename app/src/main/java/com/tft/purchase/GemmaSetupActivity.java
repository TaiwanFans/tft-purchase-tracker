...
public class GemmaSetupActivity extends Activity {
    ...
    private void render() {
        ...
        tt.addView(label("分區放大 OCR + MiniCPM｜離線辨識",14,Color.rgb(37,99,235),true));
        ...
        card.addView(label("V2.0.15｜放大分區＋辨識字典",18,Color.rgb(15,42,92),true));
        card.addView(label("採購單會先校正，再切成 H1/H2/R1/R2/R3 編號區塊；每一塊放大後由 PP-OCRv6 Medium + Google ML Kit 各自掃描，再把座標拼回完整採購單。",15,Color.rgb(71,85,105),false));
        card.addView(label("新增本機『我方公司／廠商／常購品項』資料庫。全益畜牧器具公司等我方名稱會被禁止當成供應廠商；人工修正後的廠商與品項可持續協助之後辨識。",14,Color.rgb(22,101,52),true));
        card.addView(label("MiniCPM-V 4.6 約 1.6 GB。PP-OCRv6 Medium 已包在 APK；Google 文件掃描與中文 OCR 第一次有網路時由 Play services 準備，之後可離線使用。",13,Color.rgb(100,116,139),false));
        ...
        Button knowledge=button("編輯辨識字典／廠商資料庫",Color.rgb(8,145,178));knowledge.setOnClickListener(v->startActivity(new Intent(this,KnowledgeBaseActivity.class)));root.addView(knowledge,margins(5,5));
        enter=button("進入 APP（僅查看既有資料）",Color.rgb(71,85,105));enter.setOnClickListener(v->openMain());root.addView(enter,margins(5,5));
        root.addView(label("第一次安裝建議保持 Wi-Fi，完成 MiniCPM 與 Google Play services 模組準備；之後工廠無網路仍可使用本機 OCR + AI。",13,Color.rgb(100,116,139),false));
        setContentView(root);
    }
    ...
    private void refresh() {
        ...
        if(s.state==DownloadManager.STATUS_SUCCESSFUL&&s.needsValidation){status.setText("下載 100% 完成，準備驗證模型…");progress.setProgress(100);download.setEnabled(false);startValidation();return;}
    ...
    private void startValidation(){if(validating)return;validating=true;new Thread(()->{MiniCpmV46ModelManager.ValidationResult r=MiniCpmV46ModelManager.validateDownloadedModel(this);runOnUiThread(()->{validating=false;Toast.makeText(this,r.message,Toast.LENGTH_LONG).show();refresh();});},"minicpm-md5-verify").start();}
