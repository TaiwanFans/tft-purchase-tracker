...
public class GemmaSetupActivity extends Activity {
    ...
    private void render() {
        ...
        TextView title = label("全益採購追蹤", 27, Color.rgb(15,42,92), true);
        root.addView(title);
        root.addView(label("GEMMA AI 文件辨識", 15, Color.rgb(37,99,235), true));
        ...
        card.addView(label("為什麼要安裝 Gemma？", 18, Color.rgb(15,42,92), true));
        card.addView(label("這版不再只靠 OCR。Gemma 會直接看採購單圖片，再參考 OCR 文字，協助確認採購單號、供應商、品項、數量與交貨日期。", 15, Color.rgb(71,85,105), false));
        card.addView(label("模型：約 2.6 GB，只需下載一次；下載完成後可離線使用。建議使用 Wi‑Fi。", 14, Color.rgb(220,38,38), true));
        ...
        enter = button("先進入 APP", Color.rgb(71,85,105));
        enter.setOnClickListener(v -> openMain());
        root.addView(enter, margins(8,6));

        root.addView(label("AI 模型未完成前仍可使用 OCR，但建議等 Gemma 安裝完成後再測試採購單辨識。", 13, Color.rgb(100,116,139), false));
        setContentView(root);
    }
    ...
    private void refresh() {
        if (GemmaModelManager.isReady(this)) {
            ...
            enter.setText("進入 APP（AI 辨識已啟用）");
            if (!launchedMain) {
                launchedMain = true;
                handler.postDelayed(this::openMain, 450);
            }
            return;
        }
        ...
        if (s.state == DownloadManager.STATUS_RUNNING || s.state == DownloadManager.STATUS_PENDING || s.state == DownloadManager.STATUS_PAUSED) {
            ...
            download.setEnabled(false);
            enter.setText("先進入 APP（背景繼續下載）");
        } else if (s.state == DownloadManager.STATUS_FAILED) {
            status.setText("模型下載失敗，請確認網路與手機儲存空間後重試。錯誤碼：" + s.reason);
            status.setTextColor(Color.rgb(220,38,38));
            progress.setProgress(0);
            download.setText("重新下載 Gemma AI 模型");
            download.setEnabled(true);
        } else {
            status.setText("Gemma AI 模型尚未安裝");
            progress.setProgress(0);
            download.setEnabled(true);
        }
    }
    ...
    private void openMain() {
        Intent i = new Intent(this, SafeMainActivityV204.class);
    ...
    private TextView label(String text,int sp,int color,boolean bold){
        TextView v=new TextView(this);v.setText(text);v.setTextSize(sp);v.setTextColor(color);v.setGravity(Gravity.START);v.setLineSpacing(0,1.08f);if(bold)v.setTypeface(Typeface.DEFAULT,Typeface.BOLD);v.setPadding(0,dp(5),0,dp(5));return v;
    }
    private Button button(String text,int color){Button b=new Button(this);b.setText(text);b.setAllCaps(false);b.setTextSize(15);b.setTextColor(Color.WHITE);b.setTypeface(Typeface.MONOSPACE,Typeface.BOLD);b.setBackground(box(color,Color.rgb(15,23,42),2));b.setPadding(dp(12),dp(10),dp(12),dp(10));return b;}
    private GradientDrawable box(int fill,int stroke,int width){GradientDrawable g=new GradientDrawable();g.setColor(fill);g.setStroke(dp(width),stroke);g.setCornerRadius(dp(7));return g;}
